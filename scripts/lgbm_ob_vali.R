...
if (length(args) != 12) {
  ...
  1: meses de evaluacion
  2: hiperparametros a optimizar: 'learning_rate=0.0005_0.005-num_leaves=500_800-feature_extraction=0.25_0.25-...'
  3: path features importantes: '~/features_procesadas.txt'
  4: top features importantes a usar: 10, 20, 100, ...
  5: path dataset entrada: '~/paquete_premium_201906_202001.txt.gz'
  6: carpeta salida: '~/opt_bayesiana_lgbm'
  7: número de iteraciones: 100, 200, ...
  8: undersampling: 0.01, 0.1, 1.0, ...
  9: prob corte inicial: 0.025, 0.25, ...
  10: path ganancias: '~/lgbm_ganancias.txt'
  11: numero de semillas a evaluar: 10, 100, ...
  12: correr ob: T o F",
       call.=FALSE)
}
...
path_features = args[3]
top_features = as.integer(args[4])
dataset_path = args[5]
carpeta = args[6]
n_iteraciones = as.numeric(args[7])
porcion_undersampling = as.numeric(args[8])
prob_corte_inicial = as.numeric(args[9])
ganancias_path = args[10]
n_semillas = as.integer(args[11])
correr_ob = as.logical(args[12])
...
restar_dos_meses = function(foto_mes) {
  if( foto_mes %% 100  > 2 ) {
    res = foto_mes - 2
  } else {
    res = foto_mes - 100 + 10
...
for (foto_mes_evaluacion in fotos_meses_evaluacion) {
  foto_mes_entrenamiento_desde = 201701
  foto_mes_entrenamiento_hasta = restar_dos_meses(foto_mes_evaluacion)
